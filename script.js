...
const updateSliderPos = function () {
  ...
  indicators.forEach((indicator, index) => {
    indicator.classList.toggle("active", index === currentSlidePos);
  });
}
...
// Add click event listeners for slider indicators
...
// Early definition of handleCartClick to prevent reference errors
window.handleCartClick = function() {
  console.log('Cart button clicked (early definition)');
  // This will be overridden later with full implementation
};
...
// Handle cart button click - use existing cart button from HTML
window.handleCartClick = () => {
  console.log('Cart button clicked');
  
  try {
    // If cart has items, show quick order options
    if (window.cart && window.cart.length > 0) {
      if (typeof showQuickOrderOptions === 'function') {
        showQuickOrderOptions();
  } else {
        console.error('showQuickOrderOptions not defined');
        toggleCartSidebar();
      }
    } else {
      // If cart is empty, just open the sidebar
      if (typeof toggleCartSidebar === 'function') {
        toggleCartSidebar();
      } else {
        console.error('toggleCartSidebar not defined');
      }
    }
  } catch (error) {
...
// Add cart toggle button to header - MODIFIED to avoid conflicts
const addCartToggleButton = () => {
  // Don't create new cart button since one already exists in HTML
  console.log('Cart button already exists in HTML, skipping dynamic creation');
  
  // Just ensure the cart count is updated
  updateCartCount();
};
...
window.toggleCartSidebar = () => {
  ...
  console.log('Cart sidebar element:', sidebar);
  console.log('Cart overlay element:', overlay);
  
  if (!sidebar) {
    console.error('Cart sidebar not found! Creating it now...');
    createCartSidebar();
    return;
  }
...
window.updateCartDisplay = () => {
  ...
  if (!cartItemsList) {
    console.log('Cart items list not found, cart sidebar may not be created yet');
    return;
  }
    ...
    cartItemsList.innerHTML = window.cart.map(item => `
      <div class="cart-item" data-item-id="${item.id}">
        <div class="item-image">
          <img src="${item.image || './assets/images/menu-1.png'}" alt="${item.name}" loading="lazy" onerror="this.src='./assets/images/menu-1.png'">
        </div>
  ...
  // Update totals
  const subtotal = window.cartTotal || 0;
...
// Show quick order options when cart icon is clicked
window.showQuickOrderOptions = () => {
  const totalItems = window.cart.reduce((sum, item) => sum + item.quantity, 0);
    ...
    <div id="quickOrderModal" class="quick-order-modal">
      <div class="quick-order-content">
        <div class="quick-order-header">
          <h3>🛒 Quick Order</h3>
          <button class="close-quick-order" onclick="closeQuickOrderModal()">×</button>
        </div>
        
        <div class="quick-order-summary">
          <p><strong>${totalItems} items</strong> in your cart</p>
          <p class="total-price">Total: ₹${totalPrice.toFixed(2)}</p>
        </div>
        
        <div class="quick-order-options">
          <button class="quick-order-btn view-cart-btn" onclick="viewFullCart()">
            <span class="btn-icon">👀</span>
            View Full Cart
          </button>
          
          <button class="quick-order-btn place-order-btn" onclick="quickPlaceOrder()">
            <span class="btn-icon">🚀</span>
            Place Order Now
          </button>
          
          <button class="quick-order-btn add-more-btn" onclick="addMoreItems()">
            <span class="btn-icon">➕</span>
            Add More Items
          </button>
        </div>
        ...
        <div class="quick-order-items">
          <h4>Your Items:</h4>
          ${window.cart.slice(0, 3).map(item => `
          ...
          `).join('')}
          ${window.cart.length > 3 ? `<p class="more-items">+ ${window.cart.length - 3} more items...</p>` : ''}
        </div>
      </div>
    </div>
  `;
  ...
  // Remove existing modal if any
  const existingModal = document.getElementById('quickOrderModal');
  if (existingModal) {
    existingModal.remove();
  }
...
// Authentication System (tolerant of prior declarations) - REMOVED duplicate declarations
...
const clearAllAuthData = () => {
  ...
  localStorage.removeItem('grilli_user');
  localStorage.removeItem('grilli_auth_token');
...
const initAuthSystem = () => {
  // First, clear any existing auth data to ensure clean state
  clearAllAuthData();
  
  // Check for valid authentication token first
...
// Initialize login modal (HTML modal already exists, just ensure proper styling)
const createLoginModal = () => {
  // Modal already exists in HTML, just add smooth animations and ensure proper styling
...
const handleLoginSubmit = async (e) => {
  ...
  if (failedAttempts >= 5 && lastAttempt && (now - parseInt(lastAttempt)) < 300000) { // 5 minutes
    const remainingTime = Math.ceil((300000 - (now - parseInt(lastAttempt))) / 1000);
    showAuthMessage(`Too many failed attempts. Please try again in ${remainingTime} seconds.`, 'error');
    return;
  }
  ...
  try {
    ...
    if (response.ok && data.success) {
      ...
      // Log login activity for admin portal
      ...
      setTimeout(() => {
        ...
        showWelcomeMessage(data.data.user);
      }, 1500);
      
    } else {
      // Handle failed login attempts
      const currentAttempts = parseInt(localStorage.getItem('failedLoginAttempts') || '0') + 1;
      ...
      if (currentAttempts >= 5) {
        showAuthMessage('Too many failed attempts. Please try again in 5 minutes.', 'error');
      } else {
    ...
    // Only allow specific test credentials for demo purposes
    if (email === 'demo@grilli.com' && password === 'demo123') {
      showAuthMessage('Login successful! (Demo mode)', 'success');
...
const handleRegisterSubmit = async (e) => {
  ...
  if (userData.firstName.length < 2 || userData.lastName.length < 2) {
    showAuthMessage('Name must be at least 2 characters long', 'error');
    return;
  }
  ...
  try {
    ...
    // Only allow specific demo registration
    if (userData.email === 'demo@grilli.com' && userData.password === 'demo123') {
      showAuthMessage('Registration successful! (Demo mode) - Please login with your credentials', 'success');
...
// Duplicate updateLoginButton function removed - using window.updateLoginButton

// Duplicate showUserMenu function removed to prevent conflicts
    ...
    <div id="userMenu" class="user-menu" style="display: none;">
      <div class="user-menu-content">
        ...
        <div class="user-menu-actions">
          ...
          <button onclick="viewOrders()" class="menu-action">
            <ion-icon name="receipt-outline"></ion-icon>
            My Orders
          </button>
          <button onclick="viewReservations()" class="menu-action">
            <ion-icon name="calendar-outline"></ion-icon>
            My Reservations
          </button>
          <button onclick="openAdminPortal()" class="menu-action admin-action">
            <ion-icon name="shield-outline"></ion-icon>
            Admin Portal
          </button>
          <hr>
          <button onclick="logout()" class="menu-action logout-action">
...
const viewReservations = () => {
  alert('Reservation history feature coming soon!');
};
...
window.showAuthMessage = (message, type = 'info') => {
...
const logUserActivity = async (action, data) => {
  try {
    const activityData = {
      action,
      data,
    ...
    // Send to backend for admin portal logging
    await fetch('http://localhost:3000/api/admin/log-activity', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  // Always store locally for admin portal
  ...
  localStorage.setItem('userActivities', JSON.stringify(activities.slice(-50))); // Keep last 50 activities
  }
};
...
const showWelcomeMessage = (user) => {
  ...
  welcomeMsg.innerHTML = `
    <div class="welcome-content">
      ...
      <p>You're now logged in and can access all features.</p>
      <button onclick="this.parentElement.parentElement.remove()" class="welcome-close">Got it!</button>
    </div>
  `;
...
// Authentication state - REMOVED duplicate declarations
...
// Test cart click functionality
window.testCartClick = () => {
  console.log('Testing cart click...');
  console.log('Cart button element:', document.getElementById('cartBtn'));
  console.log('handleCartClick function:', typeof window.handleCartClick);
  
  if (typeof window.handleCartClick === 'function') {
    window.handleCartClick();
  } else {
    console.error('handleCartClick function not found!');
  }
};
...
// Global authentication variables - REMOVED duplicate declarations
// Using existing window.isLoggedIn and window.currentUser
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  try {
    ...
    // Ensure login modal is hidden on page load
    const loginModal = document.getElementById('loginModal');
    if (loginModal) {
      loginModal.classList.remove('show');
      console.log('Login modal hidden on page load');
    }
...
function checkAuthStatus() {
  const token = localStorage.getItem('grilli_auth_token');
  const user = localStorage.getItem('grilli_user');
...
function updateLoginButton() {
  ...
  if (isLoggedIn && currentUser) {
    loginBtn.innerHTML = `
      <ion-icon name="person-circle-outline"></ion-icon>
      <span class="login-text">${currentUser.firstName}</span>
    `;
    loginBtn.className = 'user-profile-btn';
...
// Duplicate function removed - using the global window.closeLoginModal

// Duplicate function removed - using the global window.switchToLogin

// Duplicate function removed - using the global window.switchToRegister

// Duplicate function removed - using the global window.clearAuthMessages

// Duplicate function removed - using the global window.showAuthMessage
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (result.success) {
          // Store auth data
          window.authToken = result.data.token;
          window.currentUser = result.data.user;
          window.isLoggedIn = true;
          
          localStorage.setItem('grilli_auth_token', authToken);
          localStorage.setItem('grilli_user', JSON.stringify(currentUser));
          ...
          window.showAuthMessage(result.message || 'Login failed. Please try again.');
        }
      } catch (error) {
        console.error('Login error:', error);
        window.showAuthMessage('Network error. Please check your connection and try again.');
      } finally {
        // Reset button state
        submitBtn.innerHTML = originalText;
        submitBtn.disabled = false;
    ...
    console.error('Login form not found when adding event listener');
  }
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (result.success) {
          ...
          updateLoginButton();
          window.closeLoginModal();
          showNotification(`Welcome to Grilli, ${currentUser.firstName}!`, 'success');
          
          // Clear form
          e.target.reset();
        } else {
          window.showAuthMessage(result.message || 'Registration failed. Please try again.');
        }
      } catch (error) {
        console.error('Registration error:', error);
        window.showAuthMessage('Network error. Please check your connection and try again.');
    ...
    console.error('Registration form not found when adding event listener');
  }
});
